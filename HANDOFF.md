...
|:---:|------|:---:|:---:|:---:|------|
| **1순위** | 수강생에게 PDF 자동 첨부 메일 발송 | 쉬움 | 1시간 | 무료 | Gmail SMTP 인프라 재사용 |
| **2순위** | 디자인 보강 — 좀 더 세련된 UI/모션 | 중간 | 반나절 | 무료 | 대표님이 1차 출시 후 아쉬워하신 부분 |
| **3순위** | 시트 응답을 숙련도/매물별 자동 분류 (강의반 배정용) | 중간 | 1~2시간 | 무료 | Apps Script 또는 Sheets 수식 |
| **4순위** | 재진단 비교 리포트 — 한 달 후 진행도 비교 | 중간 | 반나절 | 무료 | 이메일 기준 lookup |
| **5순위** | 결과에 추천 유튜브·블로그 콘텐츠 큐레이션 | 중간 | 1일 | 무료 | 매핑 테이블 + 수동 큐레이션 |
| **6순위** | LLM API로 custom_goals 자유입력 자동 분석·맞춤 코멘트 | 어려움 | 1일 | 유료 (소량) | Claude/GPT API |
| **7순위** | 베타 테스터 피드백 반영 (수강생 응답 후) | 가변 | 가변 | 무료 | 베타 결과에 따라 |
